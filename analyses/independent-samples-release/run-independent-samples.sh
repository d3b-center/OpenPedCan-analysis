...
# uses histologies-base.tsv (Default), else runs all module with histologies-base.tsv

set -e
set -o pipefail

# If set to 1 (e.g., to generate files for a release), use the base histologies file
RUN_FOR_RELEASE=${OPENPBTA_BASE_RELEASE:-1}
...
Rscript 01-generate-independent-dna-specimens.R \
  -f $HISTOLOGY_FILE \
  -o results
