...
histology <- base_histology %>% 
  ...
  left_join(compiled_subtyping,by=c("Kids_First_Biospecimen_ID", "match_id", "sample_id","Kids_First_Participant_ID","tumor_descriptor"),suffix=c(".base",".subtyped")) %>%
  unique() %>%
  dplyr::mutate(
...
```{r}
histology %>% 
  filter(cohort %in% c("GMKF", "PBTA", "DGD",
...
histology_cg <- histology %>%
  ...
  dplyr::mutate(cancer_group = case_when(
    ...
    pathology_diagnosis %in% c("Astrocytoma", "Glioblastoma", "Astrocytoma;Oligoastrocytoma") ~ pathology_diagnosis,
    ...
    cancer_group %in% c("congenital malignant brain tumor", "High-grade glioma/astrocytoma (WHO grade III/IV)") ~ "High-grade glioma",
    ...
    grepl("xanthogranuloma", pathology_free_text_diagnosis) & sample_id != "7316-188" ~ "Juvenile xanthogranuloma", 
  ...
  # harmonize myofibroblastic tumors - all inflammatory; take care of organ later
    mutate(short_histology = case_when(short_histology == "Myofibroblastic tumor" ~ "Inflammatory Myofibroblastic Tumor", TRUE ~ as.character(short_histology)),
           ...
           # update typo
           broad_histology = case_when(broad_histology == "Vasculaar tumor" ~ "Vascular tumor",
