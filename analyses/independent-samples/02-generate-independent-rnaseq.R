...
independent_dna_sample_df_each <- readr::read_tsv("results/independent-specimens.wgswxspanel.primary-plus.eachcohort.prefer.wgs.tsv")
independent_dna_sample_df_all <- readr::read_tsv("results/independent-specimens.wgswxspanel.primary-plus.prefer.wgs.tsv")

# Filter for tumor samples where composition is not Derived Cell Line and Patient Derived Xenograft
...
rnaseq_relapse_each_file <- file.path(out_dir, "independent-specimens.rnaseqpanel.relapse.eachcohort.tsv")
...
rnaseq_primplus_each_file <- file.path(out_dir, "independent-specimens.rnaseqpanel.primary-plus.eachcohort.tsv")
...
rnaseq_primary_all_file <- file.path(out_dir, "independent-specimens.rnaseqpanel.primary.tsv")
...
rnaseq_relapse_all_file <- file.path(out_dir, "independent-specimens.rnaseqpanel.relapse.tsv")
...
rnaseq_primplus_all_file <- file.path(out_dir, "independent-specimens.rnaseqpanel.primary-plus.tsv")
