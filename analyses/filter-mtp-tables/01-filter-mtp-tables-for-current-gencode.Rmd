...
```{r}
# load MTP targets and diseases mapping file
mtp_targets <- file.path(input_dir, "mtp-targets-mapping.tsv.gz")
mtp_diseases <- file.path(input_dir, "mtp-diseases-mapping.tsv.gz")
targets_mapping_file <- readr::read_tsv(file.path(input_dir, "mtp-targets-mapping.tsv.gz")) %>% as.data.frame() %>% tibble::as_tibble()
disease_mapping_file <- readr::read_tsv(file.path(input_dir, "mtp-diseases-mapping.tsv.gz")) %>% as.data.frame() %>% tibble::as_tibble()
```
...
filter_mtp_table <- function(mtp_file, ensg_ids, targets_mapping_file) {
  ...
  if ("Gene_Ensembl_ID" %in% colnames(mtp_table)) { 
    ...
    mtp_table %>% dplyr::filter(Gene_Ensembl_ID %in% ensg_ids) %>%
      dplyr::filter(!Gene_Ensembl_ID %in% targets_mapping_file$gene_id) %>%
    ...
    mtp_table %>% dplyr::filter(targetFromSourceId %in% ensg_ids) %>%
      dplyr::filter(!targetFromSourceId %in% targets_mapping_file$gene_id) %>%
      readr::write_tsv(file.path(results_dir, file_name))
...
gene_level_snv <- filter_mtp_table(
  file.path(snv_results_dir, 
            "gene-level-snv-consensus-annotated-mut-freq.tsv.gz"), ensg_ids, targets_mapping_file) 

# Ensembl gene identifiers removed
filtered_ensg <- gene_level_snv %>% 
  dplyr::filter(!targetFromSourceId %in% setdiff(ensg_ids, targets_mapping_file$gene_id)) %>% 
...
variant_level_snv <- filter_mtp_table(
  file.path(snv_results_dir, 
            "variant-level-snv-consensus-annotated-mut-freq.tsv.gz"), ensg_ids, targets_mapping_file) 
...
gene_level_cnv <- filter_mtp_table(
  file.path(cnv_results_dir, 
            "gene-level-cnv-consensus-annotated-mut-freq.tsv.gz"), ensg_ids, targets_mapping_file) 
...
fusion <- filter_mtp_table(
  file.path(fusion_results_dir, 
            "putative-oncogene-fusion-freq.tsv.gz"), ensg_ids, targets_mapping_file) 
...
fused_gene <- filter_mtp_table(
  file.path(fusion_results_dir, 
            "putative-oncogene-fused-gene-freq.tsv.gz"), ensg_ids, targets_mapping_file) 
...
tpm_group_wise_stats <- filter_mtp_table(
  ...
  ensg_ids, targets_mapping_file)

# Ensembl gene identifiers removed
filtered_ensg <- tpm_group_wise_stats %>% 
  dplyr::filter(!Gene_Ensembl_ID %in% setdiff(ensg_ids, targets_mapping_file$gene_id)) %>% 
