...
alteration_df <- alteration_df %>% 
  dplyr::mutate(molecular_subtype = 
           case_when(
             ...
             ## for PPTC and Maris sample, MYCN status is in pathology_free_text_diagnosis
...
clinical_amplified_match_id <- alteration_df %>% 
  dplyr::filter(MYCN_CN_status %in% mycn_status, 
                pathology_free_text_diagnosis == "MYCN amp") %>% 
  dplyr::pull(match_id)
...
alteration_df <- alteration_df %>% 
  dplyr::filter(!match_id %in% clincal_amplified_df$match_id) %>%
...
```{r df subtyped by tmp}
 alteration_tmp_df <- alteration_df %>% 
  dplyr::filter(match_id %in% 
                  unique(c(clinical_amplified_match_id, case_5_match_id))) %>% 
...
molecular_subtype_table <- hist_df %>% 
  ...
  dplyr::select(Kids_First_Biospecimen_ID, match_id, molecular_subtype_methyl) %>%
  dplyr::rename("Kids_First_Biospecimen_ID_Methyl" = "Kids_First_Biospecimen_ID") %>% 
  dplyr::distinct() %>% 
  dplyr::full_join(molecular_subtype_table, 
                   by = "match_id") %>%
