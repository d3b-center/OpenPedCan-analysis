...
```{r df subtyped by tmp}
 alteration_tmp_df <- alteration_df %>% 
  ...
  dplyr::select(Kids_First_Biospecimen_ID_DNA, 
		...
		sample_id, cohort, MYCN_CN,
                pathology_free_text_diagnosis, 
                MYCN_CN_status, MYCN_TPM,
                molecular_subtype, match_id)
...
molecular_subtype_table <- alteration_df %>% 
  dplyr::select(Kids_First_Biospecimen_ID_DNA, 
                Kids_First_Biospecimen_ID_RNA,
                Kids_First_Participant_ID,
                sample_id, cohort, molecular_subtype, match_id) %>% 
  dplyr::mutate(molecular_subtype = 
                  case_when(
                    molecular_subtype == 
...
# include methylation samples
molecular_subtype_table <- hist_df %>% 
  dplyr::filter(pathology_diagnosis %in% path_dx_list$exact_path_dx,
                experimental_strategy == "Methylation", 
                sample_type == "Tumor") %>% 
  dplyr::mutate(molecular_subtype_methyl = NA_character_) %>% 
  ...
  dplyr::distinct() %>% 
  dplyr::right_join(molecular_subtype_table, 
                   by = "match_id") %>%
  dplyr::select(Kids_First_Participant_ID, sample_id, cohort, Kids_First_Biospecimen_ID_DNA, Kids_First_Biospecimen_ID_RNA,
...
### convert the output to long table

```{r}
proteo_hist <- hist_df %>% 
  filter(experimental_strategy %in% c("Whole Cell Proteomics", "Phospho-Proteomics")) %>% 
  select(Kids_First_Biospecimen_ID, match_id) %>%
  dplyr::rename(Kids_First_Biospecimen_ID_prot = Kids_First_Biospecimen_ID)

molecular_subtype_table_long <- molecular_subtype_table %>% 
  left_join(proteo_hist, by = "match_id") %>% 
  pivot_longer(!c("Kids_First_Participant_ID", "sample_id", "cohort", "molecular_subtype", "molecular_subtype_methyl","match_id"), 
               values_to = "Kids_First_Biospecimen_ID") %>% 
  filter(!is.na(Kids_First_Biospecimen_ID)) %>% 
  select(-c("name", "cohort")) %>% 
  distinct()
