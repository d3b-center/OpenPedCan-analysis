...
hist_methyl <- hist %>%
  ...
  filter(grepl("PB", molecular_subtype)) %>%
  filter(!grepl("To be classified", molecular_subtype)) %>%
  # update one with pathogenic germline variant
  mutate(molecular_subtype = case_when(Kids_First_Biospecimen_ID_methyl == "BS_SG2X2XQB" ~ "PB, Group 1 + DROSHA GPV",
         TRUE ~ molecular_subtype),
         molecular_subtype_methyl = case_when(Kids_First_Biospecimen_ID_methyl == "BS_SG2X2XQB" ~ "PB, Group 1A + DROSHA GPV",
...
pineo_methyl <- methyl[,colnames(methyl) %in% c("Probe_ID", hist_methyl$Kids_First_Biospecimen_ID_methyl)]
