...
fusion_file <- file.path(file.path(data_dir, 
                                   "fusion_summary_embryonal_foi.tsv"))

# subset files
manta_file <- file.path(subset_dir, "embryonal_manta_sv.tsv")
exp_file <- file.path(subset_dir, "embryonal_zscored_exp.rds")
...
# file that contains the methylation classification subtypes
...
fusions_summary_df <- list(ttyh1_fusions,
  ...
  left_join(histologies_df %>% select(Kids_First_Biospecimen_ID, match_id), by = "Kids_First_Biospecimen_ID")
...
exp_df <- exp_dat[, exp_genes_of_interest] %>%
  as.data.frame() %>%
  tibble::rownames_to_column("Kids_First_Biospecimen_ID") %>% 
  left_join(histologies_df %>% 
              select(Kids_First_Biospecimen_ID, match_id), by = "Kids_First_Biospecimen_ID")


# remove data.frame we no longer need
rm(exp_dat, exp_genes_of_interest)
```
...
```{r}
rna_df <- exp_df %>%
  left_join(fusions_summary_df, by = c("Kids_First_Biospecimen_ID", "match_id")) 
...
Generally that comes from small or zero log2(TPM + 1) values.
...
bcor_sv_df <- manta_sv_df %>%
  filter(str_detect(Gene_name, "BCOR"),
         FILTER == "PASS",
         ALT == "<DUP:TANDEM>")
...
```{r}
# relevant embyronal clinical metadata
...
# get RNA fusion panel data
emb_panel <- relevant_clinical_data %>%
  filter(experimental_strategy == "Targeted Sequencing",
         !is.na(RNA_library)) %>%
  select(Kids_First_Biospecimen_ID, match_id) %>%
  rename(Kids_First_Biospecimen_ID_Panel = Kids_First_Biospecimen_ID)

# get relevant embyronal dna clinical metadata
emb_dna <- relevant_clinical_data %>%
  filter(experimental_strategy %in% c("WGS", "WXS", "Targeted Sequencing"),
         is.na(RNA_library)) %>%
  select(Kids_First_Biospecimen_ID, match_id) %>%
  # add dna molecular data
  left_join(chr19_cn_df, by = "Kids_First_Biospecimen_ID") %>% 
  rename(Kids_First_Biospecimen_ID_DNA = Kids_First_Biospecimen_ID)

# get relevant embyronal rna clinical metadata
emb_rna <- relevant_clinical_data %>%
  filter(experimental_strategy == "RNA-Seq") %>%
  select(Kids_First_Biospecimen_ID) %>%
  # add rna molecular data
  left_join(rna_df, by = "Kids_First_Biospecimen_ID") %>% 
  rename(Kids_First_Biospecimen_ID_RNA = Kids_First_Biospecimen_ID)
  
# merge all relevant embyronal dna and rna 
all_data_df <- full_join(emb_dna, emb_rna, by = "match_id") %>% 
  full_join(emb_panel) %>%
  left_join(relevant_clinical_data %>% 
              select(Kids_First_Participant_ID,
                     sample_id, composition, match_id,
                     age_at_diagnosis_yrs,
                     germline_sex_estimate,
                     primary_site,
                     pathology_diagnosis,
                     pathology_free_text_diagnosis), by = "match_id") %>%
  distinct()
...
all_data_df %>%
  select(Kids_First_Participant_ID, 
         ...
         Kids_First_Biospecimen_ID_Panel,
         age_at_diagnosis_yrs,
         germline_sex_estimate,
         pathology_diagnosis,
...
etmr_nos_df <- all_data_df %>%
  filter(!match_id %in% etmr_c19mc_df$match_id,
         (pathology_diagnosis == "Embryonal Tumor with Multilayered Rosettes (ETMR)" | grepl("etmr|ETMRembryonal tumor with abundant neuropil and true rosettes|embryonal tumor with multilayer rosettes", pathology_free_text_diagnosis)) |
         (LIN28A > 3 & TTYH1_fusions == "None")) %>%
  select(contains("ID")) %>%
...
other_nbl_df <- all_data_df %>%
  filter(
    ...
    !(match_id %in% nb_foxr2_df$match_id)
  )

other_nbl_df
```

We're going to use the `match_id` to exclude from the subtyping table.

```{r}
exclude_match_id <- other_nbl_df %>%
  pull(match_id)
...
subtypes_df <- all_data_df %>%
  ...
  filter(!(match_id %in% exclude_match_id)) %>%
...
If CNS Embryonal, NOS", replace with high confidence methylation subtypes

```{r}
subtypes_df <-  readr::read_tsv(methyl_file) %>% 
  dplyr::full_join(subtypes_df, by = c("Kids_First_Participant_ID", "match_id", "sample_id")) %>% 
  dplyr::mutate(molecular_subtype = case_when(
    molecular_subtype == "CNS Embryonal, NOS" & !is.na(molecular_subtype_methyl) ~ molecular_subtype_methyl,
    is.na(molecular_subtype) & !is.na(molecular_subtype_methyl) ~ molecular_subtype_methyl,
    is.na(molecular_subtype) & is.na(molecular_subtype_methyl) ~ "CNS Embryonal, NOS", 
    TRUE ~ molecular_subtype)) %>% 
  dplyr::select(Kids_First_Participant_ID, match_id, sample_id,
                Kids_First_Biospecimen_ID_DNA, Kids_First_Biospecimen_ID_RNA,
                Kids_First_Biospecimen_ID_Methyl, molecular_subtype, molecular_subtype_methyl) %>% 
  dplyr::distinct()
