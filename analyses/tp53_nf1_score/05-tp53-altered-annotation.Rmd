...
consensus_tp53_snv_indel <- data.table::fread(
  file.path(data_dir,"v10/snv-consensus-plus-hotspots.maf.tsv.gz"),
                                   select = c("Chromosome",
                                              "Start_Position",
                                              "End_Position",
                                              "Strand",
...
expr_combined <- readRDS(file.path(data_dir,"v10/gene-counts-rsem-expected_count-collapsed.rds"))
...
for(i in 1:length(rna_library_list)) {
  ...
  ggviolin(subset_expr_combined, x = "tp53_altered", y = "TP53", 
         color = "tp53_altered", 
         palette = "jco",
  ...
  ggsave(file.path(plot_dir, paste0("tp53_expression_by_altered_status_", library_each, ".png")))
}
...
```{r, warning=FALSE, message=FALSE}
# plot of scores across tp53 altered status by cancer predisposition
plyr::d_ply(tp53_alterations_score_wide %>%
