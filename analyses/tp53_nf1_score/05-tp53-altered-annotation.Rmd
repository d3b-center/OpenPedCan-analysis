...
**TP53 altered - loss**, if :
 ...
 - A sample contains a TP53 hotspot SNV mutation. (Cancer hotspot database and downloadable file available). Please also crosscheck that all mutations from this table are included. Source: https://cavatica.sbgenomics.com/u/kfdrc-harmonization/kf-reference-pipeline/files/#q?search=hot
...
# Cancer database
hotspot_database_2017_tp53_snv <- read_tsv(file.path(input_dir,"protein_snv_cancer_hotspots_v2.ENS105_liftover.tsv")) %>%
  filter(Hugo_Symbol == "TP53")
hotspot_database_2017_tp53_indel <- read_tsv(file.path(input_dir,"protein_indel_cancer_hotspots_v2.ENS105_liftover.tsv")) %>%
  filter(Hugo_Symbol == "TP53")
...
cnv_domain_overlap <- read_tsv(file.path(results_dir, "loss_overlap_domains_tp53.tsv")) %>%
  ...
  left_join(histology[,c("Kids_First_Biospecimen_ID", "match_id")], by = "Kids_First_Biospecimen_ID") %>%
...
tumor_only_snv <- data.table::fread(file.path(data_dir, "snv-mutect2-tumor-only-plus-hotspots.maf.tsv.gz"), 
                                    select = c("Chromosome",
                                              "Start_Position",
                                              "End_Position",
                                              "Strand",
...
PF08563 <- bioMartDataPfamTp53 %>%
  filter(pfam_id == "PF08563") %>%
  select(domain_start, domain_end)

PF00870 <- bioMartDataPfamTp53 %>%
  filter(pfam_id == "PF00870") %>%
  select(domain_start, domain_end)

PF07710 <- bioMartDataPfamTp53 %>%
  filter(pfam_id == "PF07710") %>%
  select(domain_start, domain_end)
...
consensus_tp53_snv_indel_match <- consensus_tp53_snv_indel %>%
  ...
  mutate(
    ...
    overlap_domain = case_when(Variant_Classification %in% c("Frame_Shift_Del", "In_Frame_Del", "Nonsense_Mutation") & PF08563$domain_start >= Start_Position & Start_Position <= PF08563$domain_end ~ 1,
                               Variant_Classification %in% c("Frame_Shift_Del", "In_Frame_Del", "Nonsense_Mutation") & PF00870$domain_start >= Start_Position & Start_Position <= PF00870$domain_end ~ 1,
                               Variant_Classification %in% c("Frame_Shift_Del", "In_Frame_Del", "Nonsense_Mutation") & PF07710$domain_start >= Start_Position & Start_Position <= PF07710$domain_end ~ 1,
                                       TRUE ~ 0)
  ) 
...
tp53_alterations <- histology %>%
  ...
  dplyr::select(match_id, tp53_score, cancer_predispositions, HGVSp_Short, names(cnv_domain_overlap), names(sv_overlap), names(fusion_overlap), hotspot, activating, overlap_domain) %>%
  arrange(match_id) %>%
  unique() %>%
  replace_na(list(hotspot = 0, activating = 0, overlap_domain = 0))
...
tp53_alterations_score_wide <- tp53_alterations %>%
  mutate(cn_less_ploidy = ifelse(copy_number < ploidy, "yes", "no")) %>%
  # group 
  group_by(match_id,
  ...
  summarise(
   ...
   SNV_indel_counts= length(unique(HGVSp_Short[!is.na(HGVSp_Short)])), 
    ...
    overlap_domain = max(unique(overlap_domain[!is.na(overlap_domain) ]))) 
      ...
      case_when(
        ...
        activating == 0 &
          ...
          ( hotspot == 1  |
              ...
              # suggesting both alleles are mutated
              SNV_indel_counts > 1 |
              # framaeshift del or nonsense SNV overlaps a functional domain
              (overlap_domain >=1) |
        ...
        # if no evidence supports TP53 inactivation or activation 
        TRUE ~ "Other"
      )
  )
...
tp53_alterations_score_wide %>% 
  left_join(histology[,c("Kids_First_Biospecimen_ID", "match_id")], by = "match_id") %>%
  left_join(clinical[,c("Kids_First_Biospecimen_ID", "sample_id")], by = "Kids_First_Biospecimen_ID") %>%
